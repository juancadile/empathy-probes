...
\begin{enumerate}
  \item A functional decomposition of a costly-helping mechanism into
        ...
        interaction replicates on the post-correction sets of record
        ($+0.113$, 10/10 families); the need-conjunctive and
        moral-boundary components of this decomposition are pre-correction
        evidence pending revalidation (\S\ref{sec:resid}).
...
Norm-matched random directions applied to the same components do nothing
(writers $z=-18.4$, suppressors $z=+9.5$ against 20-draw nulls of
$\pm0.015$--$0.016$) — the effects are carried by the direction's specific
orientation, not by perturbation of these components per se.
...
($+0.019$ $[+0.003,+0.037]$ at resolved, sign-opposed): on these
pre-correction sets the writer-removal profile was consistent with
\emph{need-gated welfare value}; the sets-of-record need axis is being
revalidated before that name is reused.
...
is not yet resolved — without detectable capability cost. Stronger functional
names from the pre-correction analyses (need-gated welfare value; conjunctive
need$\times$cost gate) are used here only where revalidated on the sets of
record. The same recipe applied to a second model finds the
