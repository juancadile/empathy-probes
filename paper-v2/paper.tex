...
every price point, does not detectably move four control dispositions, and
...
(sandwich-norm-aware for Gemma-2; verified to reconstruction error
$\le 4\times10^{-4}$), yielding a moderately distributed push--pull pattern
...
(A 0.73, F 0.90) while remaining near chance on task persistence (T 0.05,
E 0.52), warmth (D 0.24), motive (G 0.55), and character (H 0.17). It
...
(diagnostic grade: 13 calibrated sources). Full ablation of the direction
...
\subsection{Held-out weight-edit effects}
On confirmatory families, writer removal shifts helping choice $-0.184$
(CI $[-0.236,-0.133]$), with all four control cells quiet (selectivity
$\ge4.3\times$) and only $\sim$14\% shrinkage from the development set.
...
($+0.019$ at resolved): the writers carry \emph{need-gated welfare value}.
...
smaller magnitude than in task-vs-welfare cells — the gate defends task
objectives, not moral incumbencies, and neither edit detectably reweights
...
headroom on the new seeds) and are reported as discovery data. Writer edits
do not transfer to games. Message content remains non-repetitive and
...
across nominally identical runs — a bf16 run-to-run noise band of
$\sim\pm0.01$ that we report as such — vs.\ $-0.077$ under chat templating);
...
pipeline. We include this section because the errors are ordinary, the
catches were cheap, and the difference between this paper and a wrong one is
the audit trail.

\section{Limitations}
\begin{itemize}
  \item \textbf{Component specificity is bounded, not absolute.} Random
        components carry a significant leak ($-0.057$ on the primary outcome;
        sampled null in E14d); the writer effect is $3.2\times$ larger.
        ``Writer''/``suppressor'' name intervention profiles on these assays,
...
selectively edit two component sets that jointly implement a costly-helping
decision: need-gated welfare value, and a conjunctive need$\times$cost gate
