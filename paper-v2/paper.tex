...
gating \emph{conjunctive on need} on the sets of record: the edit effect's
cost slope rises from $-0.02$ (need resolved) to $+0.09$ (urgent), with the
pre-registered family-paired urgent$-$resolved slope contrast at $+0.104$
$[+0.064,+0.146]$. Removing the writer set costs $\approx-0.23$ to $-0.26$
at mild and urgent need, cost-flat within need levels, and is small and
\emph{sign-opposed} at zero current need ($+0.044$ $[+0.030,+0.060]$;
paired urgent$-$resolved contrast $-0.277$ $[-0.296,-0.261]$, 0/10 families
positive) — need-gated, though the resolved-arm effect is strictly nonzero. The edits
...
\begin{enumerate}
  \item A functional decomposition of a costly-helping mechanism into
        ...
        interaction ($+0.113$, 10/10 families), the conjunctive need$\times$cost
        gate (paired slope $+0.104$), and the writer need-gating (paired mean
        $-0.277$) all replicate on the post-correction sets of record; the
        moral-boundary component remains pre-correction evidence pending
        revalidation (\S\ref{sec:resid}).
...
($+0.019$ $[+0.003,+0.037]$ at resolved, sign-opposed): the writer-removal profile is consistent with
\emph{need-gated welfare value}, and this signature REPLICATES on the
sets of record (resolved $+0.044$ / mild $-0.260$ / urgent $-0.234$; paired
urgent$-$resolved $-0.277$ $[-0.296,-0.261]$), as does the conjunctive gate
(paired slope contrast $+0.104$ $[+0.064,+0.146]$).
...
is not yet resolved — without detectable capability cost. The functional names from the pre-correction analyses — need-gated welfare
value and the conjunctive need$\times$cost gate — have now been revalidated on
the sets of record via pre-registered paired contrasts; the moral-boundary
claim has not and remains scoped to the superseded sets. The same recipe applied to a second model finds the
