...
re-derivation (\S\ref{sec:resid}). A graded need axis on the
\emph{pre-correction} sets showed the cost gating conjunctive on need (cost
slope $-0.03$ resolved $\to$ $+0.17$ urgent); the need axis for the sets of
record is being revalidated. Removing the writer set costs
$\approx-0.2$--$0.3$ at every price point, with no detectable linear welfare
cost trend of its own; on the pre-correction sets its resolved-need effect was
small and sign-opposed ($+0.019$ $[+0.003,+0.037]$), not zero. The edits
...
(direction-specificity controls for the re-derived sets are in progress and
scoped as such in \S\ref{sec:resid}).
...
($+0.019$ $[+0.003,+0.037]$ at resolved, sign-opposed): the writer-removal profile is consistent with \emph{need-gated welfare value}.
