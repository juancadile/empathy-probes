"""
Generate pairs EXCLUSIVELY for Claude Sonnet-4 and Haiku.
...
def generate_with_claude(prompt: str, model_name: str, temperature: float = 0.7) -> str:
...
def generate_pair(scenario: Dict[str, Any], model_alias: str, model_name: str, run_id: int) -> Dict[str, Any]:
    ...
    empathic_completion = generate_with_claude(empathic_prompt, model_name=model_name)
    print(f"✓ non-empathic...", end=" ", flush=True)
    non_empathic_completion = generate_with_claude(non_empathic_prompt, model_name=model_name)
    print(f"✓", flush=True)
...
def main():
    """Main function to generate Claude pairs."""
    if not ANTHROPIC_API_KEY:
    ...
    print("=" * 80)
    print("CLAUDE EXCLUSIVE DATASET GENERATION")
    print("=" * 80)
    print(f"Models: {[alias for alias, _ in MODELS]}")
        ...
        for scenario in scenarios:
            ...
            for run_id in range(TARGET_PAIRS_PER_SCENARIO_MODEL):
                ...
                try:
                    ...
                    pair = generate_pair(scenario, model_alias, model_name, run_id)
