...
- **Interpretation (claim language of record):** games evidence is **one-sided**: removing the suppression components robustly increases enacted costly helping (more engagement with the distressed user, task abandoned) in the original EIA harness; removing the positive writers produces no detectable in-game change at n=13, even though it moves the forced-choice logit metric. Plausible reading (not established): open-ended sampled behavior sits at a conversational default that positive-writer removal doesn't push below, while the suppressor edit (k=4, |z|=18.7 vs 10.0 in E14b) crosses action-selection thresholds. Bidirectionality remains established only at the forced-choice level (E13/E14b). Do not claim bidirectional *game* effects.
