...
## Frozen identification amendment (2026-07-13, before B8 fitting)

### Fixed compositional holdout and high-level-model adequacy

The alignment-training holdout is the `N=current-actual, C_text=high, O=1`
factorial combination. It is absent from all neural-alignment fitting and
hyperparameter selection, while the corresponding N=archived and lower-cost
cells remain available. It is evaluated only on B8 confirmation and cannot be
replaced by an easier combination after validation.

Code `N` as `{0,1}` and ordered active-objective textual cost as `{-1,0,+1}`
for `{zero,low,high}`. The primary high-level policy is an L2-regularized
logistic structural equation with intercept, N, cost, and N-by-cost terms,
fit on development families with `C=1` and no confirmation refit. The monotone
sensitivity constrains `b_N + b_NC*C >= 0` at every coded cost level and
`b_C + b_NC*N <= 0` at both N levels; the unconstrained model cannot rescue a
failed primary policy.

Before neural results are interpreted, the frozen high-level policy must pass
on B8 confirmation:

1. family-paired Bernoulli log-loss improvement over a development-fit
   intercept-only model has a family-clustered 95% interval above zero and
   positive LOFO means;
2. predicted action probability spans at least `.25` between the minimum and
   maximum factorial-cell means;
3. for each swapped variable, at least 8/16 families have a high-level
   interchange probability change `abs(delta_H) >= .10`, with both source
   directions represented in at least four families; and
4. the observed held-out high-cost contrast
   `Y(N=1,high) - Y(N=0,high)` is positive in at least 13/16 families and its
   family-clustered 95% interval excludes zero positively.

Failure is an inadequate high-level assay/SCM result. Do not fit a more flexible
SCM or lower headroom thresholds on B8 confirmation.

### Continuous agreement and nontrivial IIA

For each base/source interchange pair, let `delta_H` be the high-level model's
change in welfare-action probability and `delta_N` the intervened network's
change from its own unpatched base probability. The primary signed continuous
agreement is `G = sign(delta_H) * delta_N`, evaluated only where
`abs(delta_H) >= .10`; retain raw magnitudes and both swap directions. Its
family-clustered 95% interval must be above zero, at least 13/16 family means
must be positive, and every LOFO mean must remain positive.

Probability agreement must also improve relative to doing nothing: define per
pair Bernoulli KL from the frozen high-level interchanged distribution to (a)
the patched network and (b) the unpatched base network. The family-paired 95%
interval for `KL_unpatched - KL_patched` must be above zero. This baseline
improvement is required in addition to the random/shuffle comparison already
specified.

Report IIA on all eligible pairs, but the primary discrete IIA subset contains
only **flip-eligible** pairs where the high-level preferred action changes after
interchange. At least eight confirmation families must contain a flip-eligible
pair; otherwise discrete IIA is not identified and cannot support the full
claim. Primary IIA is family-balanced accuracy across the two resulting action
labels and must exceed the same metric for the unpatched network by at least
`.10`, with a family-clustered 95% interval for the improvement above zero.
The finite-null ranks remain co-required. High all-pair agreement on
non-flipping examples is descriptive only.

### Exact null generation and intervention matching

Let `root = numpy.random.SeedSequence(521066924)` and assign
`subspace_root, shuffle_root = root.spawn(2)`. Use the ordered 64 children from
each root. Random subspaces are generated by float64 standard-normal matrices
of shape `d_model x selected_dimension`, reduced QR, and deterministic column
sign canonicalization (largest-magnitude entry positive). For every receiver,
rescale the random-subspace injected vector to the selected alignment's
injected L2 norm; a norm at or below `1e-8` consumes the next deterministic
child before any outcome is read. Label-shuffle children permute variable
labels only among complete scenario-family blocks while preserving each
factorial cell count; action outcomes remain attached to their original rows
and are never shuffled. Then rerun the identical fitting and
validation-selection code.

Persist all attempted child states, bases, permutations, injected norms, and
family/control outcomes. Wrong-variable swaps use the other selected variable's
basis on the same receiver and are matched to the target injection norm. A
selected alignment must beat both 64-member null families separately under the
existing `3/65` rule; pooling the 128 nulls or choosing the easier family is not
allowed.
