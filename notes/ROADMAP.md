...
**A2.5 specificity gate (in progress):** matched non-social task choices (Cell T, 40 pairs) are complete. L19H12 is the cleanest helping-specific candidate (helping loss 0.506 vs task-choice delta 0.067). L17H13, L18H13, and L19MLP are candidate policy/tradeoff components: ablating them reduces helping while increasing task persistence. L8MLP affects both the controlled representation and helping with little task-control effect. The explicit-label V2.1 audit accepted 342/528 pairs (370 distinguishable; 92.4% intended polarity among those). Cell B's contrast largely collapses when helping is free, an informative result rather than a label to force. The accepted set is materialized under `data/contrastive_pairs/v2_1_audited/`. Remaining gates: neutral forced-choice prompts, capability/logit controls, more random controls, per-example bootstrap effects, and an independent audit sample. Promote a component to Stage B only if its empathic-decision effect exceeds these controls.
