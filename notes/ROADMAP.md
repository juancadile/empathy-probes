...
3. preserve the completed all-block WP2 development screen and calibrate its
   unstable fold-site selection with paired-family target permutations;
...
candidate. A paired-family permutation calibration now tests whether fold-site
selection converges more tightly than under a destroyed target. It cannot
calibrate the 62 apparent passes because target shuffling destroys the target
AUROC required by the screen; that requires a matched-decodability decoy.
...
- [ ] **WP2c · Matched-decodability decoy target**: define and lock a non-welfare surface/lexical target with development decodability matched to the WP3 target, then rerun the same search to calibrate whether target-plus-quiet threshold passes arise across the 1,764-config surface. The paired-label permutation cannot answer this because it destroys target signal. No decoy may be selected after inspecting its search results.
