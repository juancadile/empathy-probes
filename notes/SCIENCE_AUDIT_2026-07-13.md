...
- Before running R2b behavior, construct and seal 16 fresh scenario families with the same expressed-need x active-objective-cost estimand. Replace the old completed-task `free` boundary with an active-objective/zero-realized-loss level; keep `no active objective` separate and non-primary. Freeze all text, manipulation checks, family IDs, null-set universe, and code hashes before opening model scores. Primary inference uses fresh families only, with no interim stopping. The count is deliberately conservative: development-family effects look large, but those variance estimates are selected and norm-mismatched.
...
- Cell M itself passes the structural checks that T failed: development contains 40/40 unique text pairs across five families; M-confirm contains 48/48 across six disjoint family IDs, with explicit acknowledgment x closing Cartesian products. However, M-confirm is no longer a pristine final confirmation set: the same six families have been reused across E14c, E25, E26, fractional ablation, component controls, and audit-driven prompt/readout refinements. Existing results remain evidence, but final headline replication requires a sealed `M_confirm2` (and matched fresh task controls) with frozen current sets and protocol.
...
- Prospective probing at the prompt-final token would remove output-text leakage but not input-message lexical confounding. It must be compared against observable-state and lexical baselines and use current-vs-archived identical-quote controls; only a frozen causal action-logit/trajectory intervention can support an action-guiding claim. See `EIA_PROSPECTIVE_PREREG_2026-07-13.md`.
...
## Cross-model ablation audit

- The Gemma/Llama fractional activation-ablation comparison is procedurally comparable in its main essentials: each model uses its own development-derived M direction at an anchor-matched relative depth, evaluates the same confirmatory M/T families under raw prompting, removes fixed fractions of the unit-direction projection, and shows an approximately linear dose curve. The result supports an assay-bounded difference in dependence on one selected one-dimensional subspace.
- The accepted Gemma fractional run predates the residualized direction and therefore used the pre-correction M direction (the experiment log places E18b before E25). The exact `~88%` result has not been rerun for `direction_M_resid_block20.npy`. Their high cosine makes a qualitative reversal unlikely, but the current-direction number must be measured rather than inferred.
- `results/e17b_frac_{gemma,llama}/e17b.json` does not persist the direction path/hash, baseline per-pair scores, option orders, or the baseline mean used to compute 88%/5.5%. The script stores only model/block and delta summaries. The percentages cannot be reconstructed from those artifacts alone without joining a separate run. Fix the artifact schema before the rerun.
- Only three random directions were tested per model. Their near-zero effects are a useful geometric sanity check, not an empirical null distribution. Do not use them to support p-value or rarity language.
- Raw prompting is held constant across models, which is fair as a protocol comparison but not necessarily representative of either instruct model's deployed interface. Llama's component-edit effect changes substantially under chat templating; rerun the full-direction fraction endpoints under each model's chat template as a format-sensitivity analysis. Do not mix raw and chat denominators.
- The supported claim is narrow: on this confirmatory forced-choice assay and raw prompt format, removing the selected block-20 Gemma subspace had a much larger effect relative to its baseline margin than removing the analogous selected block-15 Llama subspace. This is not strict mediation, a conserved semantic-axis comparison, or evidence that Llama lacks the computation.

## Manipulation-check and judge audit

- `need_pretest.json` stores four sets of 20 integer ratings and means, but no judge model/version, prompt, sampled row IDs/texts, presentation order, raw responses, retry history, or timestamp. `moral_pretest.json` is weaker: it stores only need/response-now means and `n=20` per arm, with no item-level ratings at all. The experiment log names Sonnet, but the accepted artifacts cannot independently establish how the numbers were produced.
- The monotonic means are useful face-validity evidence, not a validated measurement model. A single LLM judge can share the same lexical heuristics the target model uses; unanimous resolved ratings of exactly 2 may reflect the prompt rubric or explicit phrase `nothing ... asking for` as much as latent need. Report these as one-model manipulation checks, not ground truth.
- Rebuild both pretests with a version-pinned judge, randomized/blinded arm presentation, item IDs and exact prompt persistence, raw responses, retries, and family-level summaries. Add at least one independent judge family plus a small blinded human audit. Predefine monotonicity and minimum-separation gates before rerunning.
- E27's content scorer is condition-blind in the relevant sense: the judge receives only prior user context and the player message, not baseline/suppressor or variant labels. Its SUPPORT definition is broad enough to count responsive excitement, which is appropriate for the distress-minus-control interaction. However, it uses the floating alias `claude-sonnet-5`, omits temperature/seed and API response provenance, truncates both fields to 400 characters, and currently leaves three UNKNOWNs. The existing E27 blockers therefore stand.
