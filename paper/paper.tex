...
\title{The Detection-Steering Gap in Empathy: \\
Safety Training Provides Robustness Without Preventing Manipulation}
...
\textbf{Key insight:} In our model comparison, safety training appears to provide \emph{robustness} rather than preventing manipulation---Qwen remains functional under adversarial steering while Dolphin breaks down. The suicide scenario shows unique resistance (50\% vs 87.5\% success), suggesting safety-critical content may have additional protections. This suggests RLHF might create empathy representations that are \textbf{linearly detectable but nonlinearly robust}, though more model pairs are needed to confirm this pattern.
...
\begin{tablenotes}
\small
\item $^*$ Pro-empathy steering only
\item $^{**}$ Coherence degrades at negative $\alpha$
\end{tablenotes}
\end{table}
...
\subsection{Safety Training May Provide Steering Robustness}

In our model comparison, safety training appears to provide \emph{robustness without preventing steerability}---though we note this is based on a single model pair:
