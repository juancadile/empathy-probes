...
\textbf{Contributions:}
\begin{enumerate}
\item First cross-architecture validation of empathy probes with near-perfect within-model discrimination (AUROC 0.996--1.00)
\item Discovery that empathy detection is independent of safety training, but probe directions are architecture-specific (cross-model agreement: $r=-0.06$ to $0.18$)
\item Evidence that safety training provides steering \emph{robustness} rather than preventing manipulation
