...
\title{Safety Training Creates Robust but Steerable Empathy: \\
Cross-Model Probing Reveals Asymmetric Manipulation in LLMs}
...
\textbf{Key insight:} Safety training doesn't prevent manipulation but provides \emph{robustness}---models remain functional under adversarial steering. The suicide scenario shows unique resistance (50\% vs 87.5\% success), suggesting safety-critical content has additional protections. This demonstrates that RLHF creates empathy representations that are \textbf{linearly detectable but nonlinearly robust}, with important implications for AI alignment.
...
\caption{Layer-wise steering comparison for The Listener scenario. Despite similar detection performance, layers show different steering effectiveness. Qwen (left) maintains consistent empathy levels with controlled modulation, while Dolphin (right) shows high variability and breakdown at negative alphas.}
